#!/bin/bash
# v.2024-07-16
#   FAJL="vstart";cd ~/;rm -f $FAJL.sh;wget https://raw.githubusercontent.com/BLBMS/am-t/moje/0/$FAJL.sh;chmod +x $FAJL.sh;./$FAJL.sh
sshd
...
#  sed -i "s#NAME#$NAME#g; s#POOL#$POOL#g; s#USER#$USER#g; s#PASS#$PASS#g; s#DELAVEC#$delavec#g" config.json
  ...
  sed -i "s#DELAVEC#$delavec#g" vipor.json
  
  echo -e "\n\e[0;92m Starting CCminer on FIXED POOL \e[0m\n"
  screen -ls | grep -o "[0-9]\+\." | awk "{print }" | xargs -I {} screen -X -S {} quit
  screen -ls
  screen -wipe 1>/dev/null 2>&1
  screen -dmS CCminer 1>/dev/null 2>&1
  screen -S CCminer -X stuff "~/ccminer -c ~/vipor.json\n" 1>/dev/null 2>&1
