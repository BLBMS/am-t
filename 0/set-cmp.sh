...
if [[ -z "$(getprop ro.lineage.version)" ]]; then
    ...
    alias sl='tmux ls -F "#{session_name}:#{session_id} [#{session_windows} windows] #{session_created}" | sed -E "s/CCminer/\x1b[32m&\x1b[0m/g; s/Update/\x1b[36m&\x1b[0m/g"'
    alias rr='tmux attach -t CCminer'
    alias ru='tmux attach -t Update'
...
alias ccm='~/ccminer -c ~/config.json'
